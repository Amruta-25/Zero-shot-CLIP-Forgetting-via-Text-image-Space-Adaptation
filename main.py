...
#     # precompute projections for all datasets
#     original_projections_all = []
#     final_hooks_all = []
#     all_ds_classes = []
...
#                     if (epoch + 1) % 5 == 0:
#                         with torch.no_grad():
#                             final_proj = change_hooks @ new_text_proj.weight.T
#                             epoch_logs.append({
#                                 'epoch': epoch + 1,
#                                 'forget_dist': torch.norm(proj_into - final_proj, p=2).item(),
#                                 'preserve_dist': torch.norm(preserve_output - new_preserve_output, p=2).item(),
#                                 'loss': loss.item()
#                             })
...
#             trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
#             tpr = trainable_params / total_params
#             all_logs[main_ds][forget_label]['trainable_param_ratio'] = tpr
#             all_logs[main_ds][forget_label]['epoch_logs'] = epoch_logs
...
#             # ---- Embedding drift ----
#             test_data = test_dataloaders[main_ds].dataset
#             idx_retain = [i for i, d in enumerate(test_data) if d['label'] != forget_id]
#             idx_forget = [i for i, d in enumerate(test_data) if d['label'] == forget_id]
...
#                 drift_retain = compute_embedding_drift(teacher, model, retain_loader, device, P_txt_student)
#                 drift_forget = compute_embedding_drift(teacher, model, forget_loader, device, P_txt_student)
#             all_logs[main_ds][forget_label]['drift'] = {'retain': drift_retain, 'forget': drift_forget}
